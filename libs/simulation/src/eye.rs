...
/// Where 0.0 represents how this eye can see 0% of the map
/// and 1.0 represents how this eye can stretch from the left to the right of the map
const FOV_RANGE: f32 = 0.25;

/// How wide our eye can see
const FOV_ANGLE: f32 = PI + FRAC_PI_4;

/// How many photoreceptors there are in one eye
///
/// More cells mean more crisp vision. Allowing them to be more precise in return for
/// evolution being slower or even fail
const CELLS: usize = 9;
...
impl Eye {
    ...
    pub fn process_vision(
        ...
        for food in foods {
            ...
            // turn the angle from <degree of start, degree of end> to <0, fov total angle>
            let angle = angle + self.fov_angle / 2.0;
            // take the angle and make it a ratio between 0 and 1
            let cell = angle / self.fov_angle;
            // determine which "cell" or "receptor" saw the food
            let cell = cell * (self.cells as f32);
            // Fixing edge case where a food is ALL the way to the right and we try to access idx
            // 13 of len 13
            let cell = (cell as usize).min(cells.len() - 1);

            let energy = (self.fov_range - dist) / self.fov_range;
            cells[cell as usize] += energy;
        }
